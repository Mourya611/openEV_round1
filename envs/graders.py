...
def score_action(task: TaskDefinition, action: TriageAction) -> Dict[str, float]:
    ...
    if rubric is None:
        return {
            "decision_match": OPEN_INTERVAL_EPSILON,
            "priority_match": OPEN_INTERVAL_EPSILON,
            "template_match": OPEN_INTERVAL_EPSILON,
            "keyword_coverage": OPEN_INTERVAL_EPSILON,
            "total": OPEN_INTERVAL_EPSILON,
        }

    decision_match = _clamp_open_unit_interval(
        1.0 if action.decision == rubric.decision else 0.0
    )
    priority_match = _clamp_open_unit_interval(
        1.0 if action.priority == rubric.priority else 0.0
    )
    template_match = _clamp_open_unit_interval(
        1.0 if action.response_template == rubric.response_template else 0.0
    )
    ...
    keyword_coverage = _clamp_open_unit_interval(keyword_coverage)
...
def grade_episode(task: TaskDefinition, actions: List[TriageAction]) -> float:
    ...
    for action in actions:
        if action.ticket_id in seen:
            ticket_scores.append(OPEN_INTERVAL_EPSILON)
            continue
